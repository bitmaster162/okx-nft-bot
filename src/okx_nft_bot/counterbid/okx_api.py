...
def _redact_sensitive_payload(value: Any) -> Any:
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            if lowered in {"signature", "r", "s"} and isinstance(item, str):
                redacted[key] = item[:16] + "…" if item else item
                continue
            if lowered == "protocoldata":
                if isinstance(item, str):
                    try:
                        parsed = json.loads(item)
                    except Exception:
                        redacted[key] = "<redacted protocolData>"
                    else:
                        redacted[key] = _redact_sensitive_payload(parsed)
                else:
...
class OKXAPIClient:
    ...
    def register_project_id(cls, collection_address: str, chain: str, project_id: str | int) -> None:
        """Cache a known address → project ID mapping (call from counter_bidder during scanning)."""
        ...
        *,
        ...
        for full_attempt in range(1, full_retries + 1):
            ...
            log.info("create_offer: POST %s project=%s payload=%s",
                     endpoint, project_id, json.dumps(step1_payload, default=str)[:2000])

            step1_resp = self._request(method="POST", path=endpoint, payload=step1_payload)

            resp_str = json.dumps(step1_resp, default=str)
            log.info("create_offer ← response (%d chars): %s", len(resp_str), resp_str[:3000])
            ...
            try:
                ...
                if "no longer valid" in str(exc).lower() and chain_lower in ("eth", "bsc"):
                    ...
                    try:
                        return self._create_offer_direct(
                            chain=chain_lower,
                            chain_id=chain_id,
                            account=account,
                        ...
                        log.error("create_offer: _create_offer_direct also failed: %s", direct_exc)
                        raise last_exc from direct_exc
                raise
                ...
                if items and isinstance(items[0], dict):
                    ...
                    for nested_key in ("protocolData", "collection", "contract"):
                        ...
                        if isinstance(nested, str) and nested:
                            log.info("_resolve_project_id: %s '%s' raw (str %d chars): %.300s",
                                     ep_path.split("/")[-1], nested_key, len(nested), nested)
                            try:
    ...
    # ── nftId cache (collection_address:token_id:chain → nftId string) ──
    _nft_id_cache: dict[str, str] = {}

    def _resolve_nft_id(
        self, collection_address: str, token_id: str | int, chain: str,
    ) -> str:
        """Resolve OKX internal nftId for a specific NFT by scraping its detail page.

        priapi createListing requires nftId — without it the request fails with
        "Cannot parse null string".  The nftId is NOT available from any public
        API endpoint; we obtain it from the SSR HTML of the NFT asset page:
          https://web3.okx.com/nft/asset/{chain}/{collection}/{tokenId}
        The page embeds JSON with nftInfo.id = nftId.
        """
        addr = collection_address.lower()
        tid = str(token_id)
        chain_lower = chain.lower()
        cache_key = f"{addr}:{tid}:{chain_lower}"

        cached = self._nft_id_cache.get(cache_key)
        if cached:
            return cached

        # Map chain name to OKX URL slug
        chain_slug_map = {"bsc": "bsc", "eth": "eth", "polygon": "polygon", "arbitrum": "arbitrum"}
        chain_slug = chain_slug_map.get(chain_lower, chain_lower)

        page_url = f"https://web3.okx.com/nft/asset/{chain_slug}/{addr}/{tid}"
        log.info("_resolve_nft_id: scraping %s", page_url)

        try:
            try:
                from curl_cffi import requests as http
                resp = http.get(page_url, headers={
                ...
                req = _ureq.Request(page_url, headers={
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)",
                    "Accept": "*/*",
                })
            ...
            if status != 200:
                log.warning("_resolve_nft_id: page returned %d for %s", status, page_url)
                return ""

            import re as _re
            scripts = _re.findall(
                r'<script[^>]*>(\{"appContext".*?)</script>', html, _re.DOTALL,
            )
            if not scripts:
                log.warning("_resolve_nft_id: no appContext in page %s", page_url)
                return ""

            data = json.loads(scripts[0])
            nft_info = data.get("appContext", {}).get("initialProps", {}).get("nftInfo", {})
            nft_id = str(nft_info.get("id", ""))
            if not nft_id or nft_id == "0":
                log.warning("_resolve_nft_id: no valid id in nftInfo for %s", page_url)
                return ""

            self._nft_id_cache[cache_key] = nft_id
            log.info("_resolve_nft_id: %s #%s → nftId=%s", addr[:14], tid, nft_id)
            return nft_id

        except Exception as exc:
            log.warning("_resolve_nft_id: failed for %s #%s: %s", addr[:14], tid, exc)
            return ""
    ...
    def _complete_two_step_offer(
        ...
        # ── AUTO-APPROVE: if approval status is "incomplete", approve on-chain ──
        for step in steps:
            ...
            if "approval" in action.lower():
                for approval_item in step.get("items", []):
                    if approval_item.get("status") == "incomplete":
                        kind = approval_item.get("kind", "")
                        spender = approval_item.get("approvalAddress", "")

                        if kind == "nftApproval":
                            # NFT listing requires setApprovalForAll on the collection
                            nft_addr = approval_item.get("collectionAddress", "")
                            if nft_addr and spender:
                                log.warning("create_listing: NFT approval INCOMPLETE — "
                                            "setApprovalForAll %s for %s",
                                            nft_addr[:14], spender[:14])
                                try:
                                    self._auto_approve_nft(
                                    ...
                                    log.error("create_listing: NFT approve %s FAILED: %s",
                                              nft_addr[:14], exc)
                                    raise OKXSubmitError(
                                        f"NFT approval failed for {nft_addr[:14]}: {exc}")
                        else:
                            # ERC20 currency approval
                            token_addr = approval_item.get("tokenAddress", "")
                            currency = approval_item.get("currency", "?")
                            if token_addr and spender:
                                log.warning("create_offer: %s approval INCOMPLETE — "
                                            "auto-approving %s for %s",
                                ...
                                try:
                                    ...
                                    log.error("create_offer: auto-approve %s FAILED: %s",
                                              currency, exc)
                                    raise OKXSubmitError(
                                        f"ERC20 approval failed for {currency}: {exc}")
        ...
        for attempt in range(1, max_retries + 1):
            ...
            # Redact nested signing material before logging request bodies.
            _log_body = _redact_sensitive_payload(post_body)
            log.info("create_offer step2 [attempt %d/%d]: POST %s body=%s",
                     attempt, max_retries, path, json.dumps(_log_body, default=str)[:2000])

            response = self._request(method="POST", path=path, payload=post_body)
            resp_str = json.dumps(response, default=str)
            log.info("create_offer step2 ← response: %s", resp_str[:2000])
    ...
    def _auto_approve_nft(
        self, *, nft_address: str, operator_address: str,
        private_key: str, chain_id: int,
    ) -> str:
        """Send ERC721/1155 setApprovalForAll(operator, true) on-chain. Returns tx hash."""
        from web3 import Web3
        from eth_account import Account
        ...
        tx_hash = w3.eth.send_raw_transaction(signed.raw_transaction)
        tx_hex = tx_hash.hex()
        log.info("auto_approve_nft: %s setApprovalForAll(%s) tx=%s — waiting confirmation...",
                 nft_address[:14], operator_address[:14], tx_hex)

        receipt = w3.eth.wait_for_transaction_receipt(tx_hash, timeout=60)
        if receipt["status"] != 1:
            raise RuntimeError(f"setApprovalForAll tx reverted: {tx_hex}")

        log.info("auto_approve_nft: CONFIRMED tx=%s gasUsed=%d", tx_hex, receipt["gasUsed"])
        return tx_hex
    ...
    def submit_seaport_order(
        ...
        *,
        ...
        log.info(
            "submit_seaport_order → POST %s chain=%d wallet=%s sig=%s... payload=%s",
            self._SUBMIT_ORDER_PATH, chain_id, wallet_address[:10], signature[:20],
            payload_str[:4000],
        )
        ...
        resp_str = json.dumps(response, default=str)
        log.info("submit_seaport_order ← response (%d chars): %s",
                 len(resp_str), resp_str[:3000])
    ...
    def create_listing(
        ...
        *,
        ...
        """Two-step listing via priapi createListing.

        priapi requires: numeric chain, project ID, AND nftId (OKX internal ID).
        nftId is resolved by scraping the NFT detail page HTML.
        v5 API /create-listing is dead (returns -1 "No longer available").
        """
        from eth_account import Account
        ...
        cs_collection = _W3.to_checksum_address(collection_address)
        cs_currency = _W3.to_checksum_address(currency_address)
        cs_wallet = _W3.to_checksum_address(account.address)
        ...
        if not str(project_id).isdigit():
            raise OKXSubmitError(
                f"create_listing: could not resolve project_id for {collection_address[:14]}")

        # Resolve nftId (required by priapi — without it: "Cannot parse null string")
        nft_id = self._resolve_nft_id(collection_address, token_id, chain_lower)
        if not nft_id:
            raise OKXSubmitError(
                f"create_listing: could not resolve nftId for {collection_address[:14]} #{token_id}")

        priapi_item: dict[str, Any] = {
            "project": int(project_id),
            "tokenId": str(token_id),
            "collectionAddress": collection_address.lower(),
            "price": str(price_raw),
            "currencyAddress": cs_currency,
            "count": "1",
            "source": 4,
            "validTime": valid_time,
            "nftId": nft_id,
        }
        priapi_endpoint = f"{self._CREATE_LISTING_PATH}?t={ts}"
        priapi_payload: dict[str, Any] = {
            "chain": chain_id,
            "walletAddress": cs_wallet,
            "items": [priapi_item],
        }
        log.info("create_listing: priapi POST %s project=%s token=%s nftId=%s price=%s",
                 priapi_endpoint, project_id, token_id, nft_id, price_raw)

        step1_resp = self._request(method="POST", path=priapi_endpoint, payload=priapi_payload)
        resp_str = json.dumps(step1_resp, default=str)
        log.info("create_listing priapi ← (%d chars): %s", len(resp_str), resp_str[:2000])

        code = str(step1_resp.get("code", ""))
        if code not in ("0", ""):
            # Check for errors array
            errors = step1_resp.get("data", {}).get("errors", [])
            err_msg = errors[0].get("message", "") if errors else ""
            api_msg = step1_resp.get("msg") or step1_resp.get("error_message") or err_msg or resp_str[:500]
            log.error("create_listing FAILED code=%s msg=%s", code, api_msg)
            raise OKXSubmitError(f"create_listing failed: code={code} msg={api_msg}")

        # Check for errors in successful response (code=0 but errors array non-empty)
        errors = step1_resp.get("data", {}).get("errors", [])
        if errors:
            err_msg = errors[0].get("message", "unknown error")
            log.error("create_listing: API returned errors: %s", err_msg)
            raise OKXSubmitError(f"create_listing failed: {err_msg}")

        return self._complete_two_step_offer(step1_resp, private_key, chain_id, priapi_endpoint)
    ...
    def buy_listing(
        ...
        *,
        ...
        """Buy a listed NFT via OKX v5 API.

        POST /api/v5/mktplace/nft/markets/buy
        Returns steps with TakeOrders action containing tx calldata.
        The caller must execute the on-chain transaction.
        """
        chain_lower = chain.lower()
        okx_chain = {"bsc": "bsc", "eth": "eth", "polygon": "polygon",
                      "arbitrum": "arbitrum"}.get(chain_lower, chain_lower)

        payload: dict[str, Any] = {
            "chain": okx_chain,
            "walletAddress": wallet_address,
            "items": [{"orderId": str(order_id), "takeCount": 1}],
        }

        log.info("buy_listing: POST /api/v5/mktplace/nft/markets/buy order=%s wallet=%s chain=%s",
                 order_id, wallet_address[:14], okx_chain)

        resp = self._request(
            method="POST",
            path="/api/v5/mktplace/nft/markets/buy",
            payload=payload,
        )

        resp_str = json.dumps(resp, default=str)
        log.info("buy_listing ← response (%d chars): %s", len(resp_str), resp_str[:3000])

        code = str(resp.get("code", "0"))
        if code not in ("0", ""):
            msg = resp.get("msg") or resp.get("message") or resp_str[:500]
            log.error("buy_listing FAILED code=%s msg=%s", code, msg)
            raise OKXSubmitError(f"buy_listing failed: code={code} msg={msg}")

        # Extract TakeOrders step with transaction calldata
        data = resp.get("data", resp)
        steps = data.get("steps", [])
        errors = data.get("errors", [])
        if errors:
            log.error("buy_listing errors: %s", errors)
            raise OKXSubmitError(f"buy_listing errors: {errors}")

        # Find TakeOrders step
        take_step = None
        approval_step = None
        for step in steps:
            action = step.get("action", "")
            if "TakeOrders" in action or "transaction" in str(step.get("items", [{}])[0].get("kind", "")):
                take_step = step
            elif "Approval" in action:
                approval_step = step

        if not take_step:
            log.error("buy_listing: no TakeOrders step in response. steps=%s",
                      [s.get("action") for s in steps])
            raise OKXSubmitError("buy_listing: no TakeOrders step in API response")

        # Extract transaction data from TakeOrders
        items = take_step.get("items", [])
        if not items:
            raise OKXSubmitError("buy_listing: TakeOrders step has no items")

        tx_item = items[0]
        return {
            "contract_address": tx_item.get("contractAddress", ""),
            "input": tx_item.get("input", ""),
            "value": tx_item.get("value", "0"),
            "total_price": tx_item.get("totalPrice", "0"),
            "status": tx_item.get("status", ""),
            "approval_needed": approval_step is not None and any(
                i.get("status") == "incomplete" for i in (approval_step.get("items", []))
            ),
