...
class CounterBidder:
    ...
    def scan_wallet(self) -> ScanReport:
        ...
        for chain in self.chains:
            ...
            for i, coll_addr in enumerate(wl_addrs_chain):
                ...
                # ── PATCH 2026-04-25: honor blacklist + buy_config.enabled=False in Phase 1 too ──
    ...
    def _pick_currency(self, rival_cur: str, cfg_cur: str,
        ...
        """Choose our offer currency: config > rival > native > any available."""
        cur_mapped = self._WRAP_MAP.get(rival_cur, rival_cur)
    ...
    def _undercut_collection(self, collection_address: str,
        ...
        if best_rival is None:
            ...
            outbid_usd = rival_usd * (1 + self.undercut_bps / 10000)
            ...
            cur.execute("""SELECT order_hash, price_bnb, current_floor, placed_at, status
                FROM active_offers WHERE collection=? AND chain=?
                  AND (
                      ...
                      OR
                      (status='exchange_missing' AND placed_at > datetime('now', '-6 hours'))
                  )
    ...
    def _check_balance_for_offer(self, currency_address: str, currency: str,
        ...
        try:
            ...
            # 2026-04-24: OKX offers are off-chain signatures — DO NOT lock funds.
            # Removed balance-block; keep low-balance TG alert above for awareness.
            # Funds only needed at fill-time; OKX rejects fills if insufficient.
            if balance < offer_price:
                log.info("ℹ️ %s balance=%.6f < offer=%.6f — proceeding (off-chain, no fund lock)",
        ...
        for col_addr, nfts in nfts_by_collection.items():
            ...
            if cheapest_rival_price is None:
                ...
                if our_sell_price < low_price:
                    if not has_config and low_price > cheapest_rival_price * 2:
                        log.warning(
                            "  SELL %s: low_price %.6f (from best_offer, no config) "
                            "looks phantom vs cheapest_rival=%.6f — ignoring clamp, "
                            "listing at rival-undercut %.6f",
                            col_addr[:14], low_price, cheapest_rival_price, our_sell_price,
                        )
                    else:
                ...
                if our_sell_price > floor_cap:
                    log.warning(
                        ...
                        "forcing price to rival-undercut; low_price source may be stale",
