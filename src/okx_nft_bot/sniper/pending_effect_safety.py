        ...
        *,
        ...
        if success:
            ...
            try:
                ...
                finalized = durable.complete_confirmed_success(
                    wallet=wallet,
                    chain=chain,
                    order_id=order_id,
                    tx_hash=tx_hash,
                ...
                # Retain the in-process latch if durable terminalization fails.
                # Receipt-confirmed success must never be made retryable merely
                # because local persistence could not complete.
                finalized = False
            if finalized:
                with lock:
