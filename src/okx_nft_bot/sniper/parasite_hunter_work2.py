...
class ParasiteHunter:
        ...
        for chain in self.chains:
            ...
            for i, coll_addr in enumerate(wl_addrs_chain):
                ...
                if top_offers[0].maker and top_offers[0].maker in self.protected_wallets:
                    log.info("  \u2705 %s: we're #1 (%.6f %s), %d enemies below \u2014 KEEP",
                             coll_addr[:14], top_offers[0].price, top_offers[0].currency, len(enemies))
                    if enemies:
                        log.info("    Our: %.6f %s | Best enemy: %.6f %s",
                                 top_offers[0].price, top_offers[0].currency,
                                 enemies[0].price, enemies[0].currency)
                    self.already_winning += 1
                    if i % 8 == 7:
                        time.sleep(3.0)
                    else:
    ...
    def _undercut_collection(self, collection_address: str,
        ...
        # ── TWO-TIER SYSTEM ──
        _QTY10_USD_THRESHOLD = 0.54
        is_premium = has_collection_config and cap_usd > _QTY10_USD_THRESHOLD
        # WL collections with cap <= $0.54 → qty=10, otherwise qty=1
        default_qty = 10 if (is_wl and cap_usd <= _QTY10_USD_THRESHOLD) else 1
        ...
        quantity = coll_cfg.get("max_offers", default_qty)
        # Safety: if actual offer price > $0.54, force qty=1
        if our_usd > _QTY10_USD_THRESHOLD and quantity > 1:
            log.info("  📉 %s: offer $%.2f > $%.2f — forcing qty=1", name, our_usd, _QTY10_USD_THRESHOLD)
            quantity = 1
        ...
        if our_existing:
            ...
            if existing_usd_pre > 0 and abs(our_usd - existing_usd_pre) / existing_usd_pre < 0.01:
                log.info("  ✅ %s: new $%.4f ≈ old $%.4f (diff <1%%) — KEEP, no point cancel+replace",
                         name, our_usd, existing_usd_pre)
                return HuntResult(addr, name, chain, len(offers), 0, 0, 1)
    ...
    def _run_parasite_intel(self, chain: str) -> dict:
        ...
        for wallet in sorted(self.target_wallets):
            ...
            # Count active OKX offers for this parasite wallet
            offer_count = 0
            try:
                wallet_offers = self._fetch_single_wallet(wallet, chain)
                offer_count = len(wallet_offers)
            except Exception:
                pass
