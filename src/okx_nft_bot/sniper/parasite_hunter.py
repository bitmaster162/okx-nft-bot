...
class ParasiteHunter:
    ...
    def scan_wallet(self) -> ScanReport:
        ...
        for chain in self.chains:
            ...
            for i, coll_addr in enumerate(wl_addrs_chain):
                ...
                top_offers = self._fetch_top_offers_for_collection(coll_addr, chain, limit=20)
                if not top_offers:
                    continue
                ...
                # Relative phantom filter: if top enemy is >3x second-best, drop it
                if len(enemies) >= 2:
                    e_usd = [(e, self.prices.to_usd(e.price, e.currency.upper())) for e in enemies]
                    e_usd.sort(key=lambda x: x[1], reverse=True)
                    if e_usd[1][1] > 0 and e_usd[0][1] > e_usd[1][1] * 3:
                        log.info("  🗑 %s: Phase1 filtering outlier enemy $%.2f (>3x second $%.2f) maker=%s",
                                 coll_addr[:14], e_usd[0][1], e_usd[1][1], (e_usd[0][0].maker or "?")[:14])
                        enemies = [e for e, _ in e_usd[1:]]
                ...
                if top_offers[0].maker and top_offers[0].maker in self.protected_wallets:
                    ...
                    if enemies:
                        ...
                        # Overpay check 2: we're >50% above best enemy → re-place
                        if best_en_usd > 0 and our_top_usd > best_en_usd * 1.5:
                            log.info("  ⚠ %s: we're #1 but OVERPAYING $%.2f vs enemy $%.2f (>1.5x) — will re-place",
                                     coll_addr[:14], our_top_usd, best_en_usd)
                            wl_enemy_offers.extend(enemies)
                            time.sleep(2.0)
                            continue
                        log.info("    Our: %.6f %s ($%.2f) | Best enemy: %.6f %s ($%.2f)",
                                 our_top.price, our_top.currency, our_top_usd,
                                 best_en.price, best_en.currency, best_en_usd)
                    log.info("  \u2705 %s: we're #1 (%.6f %s $%.2f), %d enemies below \u2014 KEEP",
                             coll_addr[:14], our_top.price, our_top.currency, our_top_usd, len(enemies))
                    self.already_winning += 1
                    time.sleep(2.0)
                    continue
    ...
    def _pick_currency(self, enemy_cur: str, cfg_cur: str,
                       chain_currencies: set[str], chain_native: str) -> str:
        """Choose our offer currency: config > enemy > native > any available."""
        cur_mapped = self._WRAP_MAP.get(enemy_cur, enemy_cur)
        if cfg_cur and cfg_cur in chain_currencies:
            return cfg_cur
    ...
    def _undercut_collection(self, collection_address: str,
        ...
        # First pass: collect all valid enemy offers with USD values
        enemy_candidates: list[tuple[ParasiteOffer, float]] = []
        ...
        for offer in offers:
            ...
            if phantom_ceiling > 0 and offer_usd > phantom_ceiling:
                ...
                continue
            enemy_candidates.append((offer, offer_usd))

        # Second pass: relative outlier detection (no buy_config or additional filter)
        # If top offer is >5x the second-best, it's likely a phantom.
        if len(enemy_candidates) >= 2:
            enemy_candidates.sort(key=lambda x: x[1], reverse=True)
            top_usd = enemy_candidates[0][1]
            second_usd = enemy_candidates[1][1]
            if second_usd > 0 and top_usd > second_usd * 5:
                phantom = enemy_candidates[0][0]
                log.info("  🗑 %s: filtering outlier offer $%.2f (>5x second $%.2f) maker=%s — likely phantom",
                         name, top_usd, second_usd, (phantom.maker or "?")[:14])
                enemy_candidates = enemy_candidates[1:]  # drop top

        # Pick best enemy from filtered candidates
        for offer, offer_usd in enemy_candidates:
        ...
        if best_enemy is not None:
            ...
            parasite_cur_mapped = self._WRAP_MAP.get(parasite_cur, parasite_cur)
            ...
            our_cur = self._pick_currency(parasite_cur_mapped, cfg_cur, chain_currencies, chain_native)
        else:
        ...
        # BUT only if we're already ABOVE the enemy (winning). If enemy is
        ...
        if our_existing:
            ...
            we_are_winning = existing_usd_pre >= parasite_usd
            if we_are_winning and existing_usd_pre > 0 and abs(our_usd - existing_usd_pre) / existing_usd_pre < 0.01:
                log.info("  ✅ %s: new $%.4f ≈ old $%.4f (diff <1%%), we beat enemy $%.4f — KEEP",
                         name, our_usd, existing_usd_pre, parasite_usd)
                return HuntResult(addr, name, chain, len(offers), 0, 0, 1)

        # ── Check if outbid price exceeds cap ──
        # If best enemy is above cap, try to find the next-best enemy below cap.
        ...
        if enemy_above_cap:
            # Fallback: find the best enemy BELOW cap
            fallback_enemy: ParasiteOffer | None = None
            fallback_usd = 0.0
            for o in offers:
                ...
                if 0 < o_usd < cap_usd and o_usd > fallback_usd:
                    ...
                    fallback_usd = o_usd
            if fallback_enemy:
                log.info("  🔄 %s: best enemy $%.2f > cap $%.2f — fallback to next enemy $%.4f %s (maker=%s)",
                         name, parasite_usd, cap_usd, fallback_enemy.price,
                         fallback_enemy.currency, (fallback_enemy.maker or "?")[:14])
                offer = fallback_enemy
                parasite_cur = offer.currency.upper()
                parasite_usd = fallback_usd
                # CRITICAL: recalculate our_cur for fallback enemy's currency
                # (original our_cur was set based on phantom enemy, e.g. USDT
                    ...
                    if existing_usd > 0 and existing_usd >= fallback_usd:
                        ...
                        if existing_usd <= lower_threshold:
                            ...
                            if self._needs_qty_upgrade(addr, chain, existing_usd, _fb_want):
                                ...
                                log.info("  ✅ %s: our $%.4f ≥ fallback enemy $%.4f, within 20%% — KEEP",
                                         name, existing_usd, fallback_usd)
                                self.already_winning += 1
                                return HuntResult(addr, name, chain, len(offers), 0, 0, 1)
                enemy_above_cap = False  # resolved via fallback
            elif our_existing:
                log.info("  ⏸ %s: enemy=%.2f$ ≥ cap=%.2f$, no fallback — keeping our offer (%.4f %s)",
        ...
        if our_existing:
            ...
            if _ex_usd > 0 and our_usd > 0 and abs(our_usd - _ex_usd) / _ex_usd < 0.02:
                if self._needs_qty_upgrade(addr, chain, our_usd, quantity):
    ...
    def _fetch_best_offer_maker(self, collection_address: str,
        ...
        if page_usd > 0 and best_enemy["usd"] > page_usd * 2.5:
            log.info("  🗑 %s: best_enemy $%.2f > 2.5x page_best $%.2f — phantom, clamping to page_best",
                     addr[:14], best_enemy["usd"], page_usd)
            # Clamp best_enemy to something reasonable — use page_best as guide
            # but keep it as "enemy" so undercutting still works
            best_enemy = {"maker": best_enemy.get("maker", ""), "price": 0,
                          "currency": "", "usd": 0}
    ...
    def _resolve_currency(raw_currency: str | None, chain: str,
        ...
        if c == "0x0000000000000000000000000000000000000000":
            return default
        return ParasiteHunter._KNOWN_ADDRESSES.get(c, default)
    ...
    def _run_sell_phase(self, chain: str) -> int:
        ...
        for col_addr, nfts in nfts_by_collection.items():
            ...
            if not low_price or low_price <= 0:
                if has_config:
                    # Config exists but no price fields → safe default
                    low_price = _SELL_DEFAULT_PRICE
                else:
                    # No config at all → check live offers to find market floor
                    best_offer_price = self._get_best_offer_price(
                        client, chain, col_addr, sell_decimals_pre)
                    if best_offer_price and best_offer_price > 0:
                        # Don't sell below best offer — use it as floor
                        low_price = best_offer_price
                        log.info("  SELL %s: no config → using best offer %.6f as floor",
                                 col_addr[:14], low_price)
                    else:
                        # No offers either → safe high default
                        low_price = _SELL_DEFAULT_PRICE
            ...
            log.info("  SELL %s: %d our listings, cheapest_enemy=%s",
                     col_addr[:14], len(our_listings),
                     f"{cheapest_enemy_price:.6f}" if cheapest_enemy_price else "NONE")
