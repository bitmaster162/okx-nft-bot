...
def install_defense_fanout_safety(engine_class: type[Any]) -> None:
    ...
    def guarded_apply_action(self: Any, action: Any) -> None:
        ...
        # Direct _apply_action() calls outside run_cycle preserve legacy behavior.
        if seen is None:
            original_apply_action(self, action)
            return
        ...
        # Mark before delegating: even a blocked/failed first action consumes the
        # one-per-cycle slot, preventing a second active row from escalating into
        # another external attempt during the same cycle.
        seen.add(collection)
        original_apply_action(self, action)
