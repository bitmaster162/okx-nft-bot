...
class OKXSalesClient:
    ...
    def normalize(self, trade: dict, parasite_wallets: set[str]) -> SaleEvent:
        ...
        return SaleEvent(
            ...
            currency=trade.get("currency") or trade.get("currencyName") or trade.get("tokenSymbol") or "ETH",
            seller=seller,
            buyer=buyer,
            tx_hash=tx,
...
class OpenSeaSalesClient:
    ...
    def normalize(self, event: dict, parasite_wallets: set[str]) -> SaleEvent:
        ...
        return SaleEvent(
            ...
            price=price,
            price_usd=None,
            currency=payment.get("symbol") or "ETH",
            seller=seller,
            buyer=buyer,
...
class MagicEdenSalesClient:
    ...
    def fetch_recent_sales(self, *, contract: str,
        ...
        chain_map = {"bsc": "bsc", "ethereum": "ethereum", "polygon": "polygon"}
        chain_slug = chain_map.get(self.chain, self.chain)
    ...
    def normalize(self, sale: dict, parasite_wallets: set[str]) -> SaleEvent:
        ...
        if isinstance(price_raw, dict):
            ...
            currency = price_raw.get("currency", {}).get("symbol") or "BNB"
        else:
...
class TelegramAlerter:
    ...
    def alert_binance_trade(self, event: SaleEvent, wl_info: dict,
        ...
        # Trade type emoji + label
        _TYPE_ICONS = {
            "покупка": "🛒",   # green cart = buy/sale
            "листинг": "🏷️",  # tag = listing
            "офф":     "💎",   # gem = offer accepted
        }
        type_icon = _TYPE_ICONS.get(event.trade_type, "📊")
...
class SalesStreamDaemon:
    ...
    def get_buy_limits(self, collection_address: str, chain: str) -> dict | None:
        """Get max buy/offer prices for a collection.

        Checks per-collection override first, then falls back to chain defaults.
